...
must_haves:
  truths:
    ...
    - "Graceful degradation: if price data completely unavailable (Get30DayHighAsync or Get200DaySmaAsync returns 0), that component uses 1.0x; if calculation throws exception, entire multiplier falls back to 1.0x with warning — stale data is always used per CONTEXT.md, never rejected"
...
<task type="auto">
  ...
  <action>
    ...
    **Add private helper method CalculateMultiplierAsync:**
    ...
    - **Handle 0-means-unavailable contract from PriceDataService:**
      - If `high30Day == 0`: skip dip calculation entirely, set dipMultiplier = 1.0m, dropPercent = 0, tier = "N/A (no price data)". Log warning: "30-day high unavailable (returned 0), using 1.0x dip multiplier"
      - If `high30Day > 0`: calculate dip percentage = `(high30Day - currentPrice) / high30Day * 100m`
    ...
    - **Handle 0-means-unavailable for bear boost:**
      - If `ma200Day == 0`: skip bear boost entirely, set bearMultiplier = 1.0m. Log warning: "200-day SMA unavailable (returned 0), skipping bear boost"
      - If `ma200Day > 0 && currentPrice < ma200Day`: bearMultiplier = options.BearBoostFactor
      - If `ma200Day > 0 && currentPrice >= ma200Day`: bearMultiplier = 1.0m (not in bear territory)
