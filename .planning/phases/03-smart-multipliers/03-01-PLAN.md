...
<task type="auto">
  ...
  <action>
    ...
    **Update Purchase.cs** - add multiplier metadata fields:
    ...
    - `public decimal DropPercentage { get; set; }` — % drop from 30-day high (non-nullable, defaults to 0 meaning "not calculated"; Phase 2 purchases already default to 0 which is correct)
    - `public decimal High30Day { get; set; }` — 30-day high used for calculation (non-nullable, defaults to 0 meaning "not calculated")
    - `public decimal Ma200Day { get; set; }` — 200-day SMA used for calculation (non-nullable, defaults to 0 meaning "not calculated")
    - Keep existing `Multiplier` field (already exists at 1.0m default)
    - **Design note:** Non-nullable decimal fields with default 0 are intentional. In C#, `decimal` defaults to 0. For old Phase 2 purchases without multiplier data, 0 correctly means "not calculated." This is NOT nullable-vs-non-nullable ambiguity — 0 is the sentinel for "no data." MultiplierTier is `string?` because null is the natural sentinel for strings.
