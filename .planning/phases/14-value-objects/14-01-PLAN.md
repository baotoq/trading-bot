...
must_haves:
  truths:
    ...
    - "PurchaseCompletedEvent carries typed value objects instead of raw decimals"
    - "Dashboard JSON API serializes value objects as raw primitives (no wrapping objects)"
  artifacts:
...
<task type="auto">
  ...
  <action>
    **Purchase.cs** -- Replace decimal fields with value objects:
    ...
    - `decimal High30Day` -> `Price High30Day`
    - `decimal Ma200Day` -> `Price Ma200Day`
    ...
    **PurchaseCompletedEvent.cs** -- Replace decimal fields:
    - `decimal BtcAmount` -> `Quantity BtcAmount`
    - `decimal Price` -> `Price Price`
    - `decimal UsdSpent` -> `UsdAmount UsdSpent`
    - `decimal RemainingUsdc` -> `UsdAmount RemainingUsdc`
    ...
    **DashboardDtos.cs** -- Keep all DTO fields as raw primitives (`decimal`, `Guid`). DTOs are the API boundary. Per Phase 13 precedent (`PurchaseDto.Id` stays `Guid`), dashboard DTOs use raw types. Vogen's implicit cast handles the conversion in LINQ Select projections. No changes needed to DashboardDtos.cs beyond ensuring it compiles with the entity changes.
...
- PurchaseCompletedEvent uses typed fields
