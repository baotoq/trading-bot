...
must_haves:
  truths:
    ...
    - "Every SUMMARY.md across phases 26-30 has a requirements-completed field in YAML frontmatter (between --- delimiters) listing its satisfied requirement IDs"
...
<objective>
Create VERIFICATION.md for phases 26-29, add requirements-completed YAML frontmatter to phases 27-28 SUMMARY files, and close all REQUIREMENTS.md traceability checkboxes.

Purpose: Formally verify all 20 v4.0 requirements are satisfied with evidence, completing the milestone verification gate.
Output: 4 new VERIFICATION.md files, 4 SUMMARY.md files updated with requirements-completed YAML frontmatter, REQUIREMENTS.md fully closed.
Note: Phase 30 VERIFICATION.md already exists (created during phase 30 execution) — only phases 26-29 need new VERIFICATION.md files.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **Phase 29 VERIFICATION.md** — Requirements: DISP-01 through DISP-10
    ...
    These files currently have NO YAML frontmatter (they start with `# Phase ...`).
    Add YAML frontmatter at the very top of each file, consistent with the phase 26/29/30 pattern:

    - `27-01-SUMMARY.md`: Insert at the top of the file:
      ```
      ---
    ...
    - `27-02-SUMMARY.md`: Insert at the top of the file:
      ```
      ---
    ...
    - `28-01-SUMMARY.md`: Insert at the top of the file:
      ```
      ---
    ...
    - `28-02-SUMMARY.md`: Insert at the top of the file:
      ```
      ---
      requirements-completed: [PORT-04, PORT-05]
      ---
    ...
    The pattern MUST be YAML frontmatter (between `---` delimiters), NOT a markdown section. This matches how phases 26, 29, and 30 store `requirements-completed`.
  </action>
  <verify>
    All 4 VERIFICATION.md files exist and have valid YAML frontmatter with status: passed.
    All 11 SUMMARY.md files across phases 26-30 have `requirements-completed:` in YAML frontmatter (verify with `grep 'requirements-completed:' <file>`).
  ...
  <done>
    ...
    4 SUMMARY.md files updated with `requirements-completed` in YAML frontmatter (consistent with phase 26/29/30 pattern).
...
2. `grep -r 'requirements-completed:' .planning/phases/2[6-9]-*/*-SUMMARY.md .planning/phases/30-*/*-SUMMARY.md` — all 11 SUMMARY files have the YAML frontmatter field (colon suffix ensures YAML key, not markdown heading)
