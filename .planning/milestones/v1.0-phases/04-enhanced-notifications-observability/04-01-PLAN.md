...
<task type="auto">
  ...
  <action>
0. **DcaOptions.cs** — Verify `DryRun` property exists (added in Phase 3). Confirm line: `public bool DryRun { get; set; } = false;` is present. No changes needed — this step is a pre-flight check to ensure the property Task 2 depends on is available.
...
4. **Create EF Core migration** — Run:
  ...
  <verify>
    - `DcaOptions.cs` contains `public bool DryRun { get; set; } = false;` (pre-flight confirmation)
...
3. **Idempotency check bypass for dry-run:**
   In Step 2 (idempotency check), wrap the entire idempotency block in `if (!options.DryRun)` so dry-run always executes regardless of existing purchases:
   ```csharp
   ...
   ```
   This replaces the existing idempotency block. The existing query (without IsDryRun filter) stays intact inside the guard, so real-mode behavior is unchanged. Dry-run mode bypasses entirely, allowing repeated simulated runs on the same day.
...
- Idempotency check bypassed entirely in dry-run mode (wrapped in `if (!options.DryRun)`)
