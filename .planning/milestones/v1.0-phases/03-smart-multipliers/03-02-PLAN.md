...
must_haves:
  truths:
    ...
    - "Stale data policy (CONTEXT.md override of FR-7): use last known values even if >24h old — NEVER reject data or fall back to 1x due to staleness"
    - "Return value contract: Get30DayHighAsync and Get200DaySmaAsync return 0 to signal 'data unavailable' (not literal zero price); callers must check for 0 before using in calculations"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **Key design decisions:**
    ...
    - **Return value contract: 0 means "data unavailable," not a literal zero price.** BTC price is never 0, so this is an unambiguous sentinel. Callers MUST check for 0 before using return values in calculations:
      - `Get30DayHighAsync` returns 0 → caller skips dip percentage calculation (uses 1.0x dip multiplier)
      - `Get200DaySmaAsync` returns 0 → caller skips bear boost comparison (uses 1.0x bear multiplier)
      - This prevents division-by-zero when computing `(high30Day - currentPrice) / high30Day` and prevents nonsensical comparisons like `currentPrice < 0`
    - **Stale data policy (CONTEXT.md override of FR-7):** Use last known values in DB even if >24h old — NEVER reject data based on age and NEVER fall back to 1x due to staleness. Background refresh handles keeping data current. If data is stale, the PriceDataRefreshService will attempt a background refresh on its next cycle, but stale data is always preferred over no data.
